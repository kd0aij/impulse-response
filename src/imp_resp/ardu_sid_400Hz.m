...
# load the measured rates and rate setpoints
[ts, rate0, rate_sp0, att0, att_sp0, data] = loadSID(basePath);

# rotate gyro readings into body frame
if false
  rtheta = pi/4;
  rotMatrix = [cos(rtheta) -sin(rtheta); sin(rtheta) cos(rtheta)]
  rate0(:,1:2) = rate0(:,1:2) * rotMatrix;
endif

nsamples = size(rate0)(1);
sigRange = [1:nsamples];
...
plot(ts, rate0(:,1), "-b", ts, rate0(:,2), "-r");
...
while (true)
  subplot(nplots,1,2);
  plot(ts(sigRange), rate0(sigRange,1), "-b", ts(sigRange), rate0(sigRange,2), "-r");
...
%    startOffSP = 1;
%    while (ts(startOffSP) < startTime) startOffSP++; endwhile
%    endOffSP = startOffSP;
%    while (ts(endOffSP) < endTime) endOffSP++; endwhile
%    sigRangeSP = [startOffSP:endOffSP];
%  endif
endwhile

# resample to uniform rate, over the time range [startTime, endTime]
[g0tu, rate0u] = resample2(startTime, endTime, ts, rate0, sampRate, false);
[s0tu, rate_sp0u] = resample2(startTime, endTime, ts, rate_sp0, sampRate);
...
%gyroDot0u = ddt(rate0u);

nsamples = size(rate0u)(1);
sigRange = [1:nsamples];

rollRateSig = rate0u(sigRange,1);
%rollAccelSig = gyroDot0u(sigRange,1);
rollSetpoint = rate_sp0u(sigRange,1);

pitchRateSig = rate0u(sigRange,2);
%pitchAccelSig = gyroDot0u(sigRange,2);
pitchSetpoint = rate_sp0u(sigRange,2);
